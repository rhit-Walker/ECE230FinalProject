 ...
 *         /|\|                  |
 *          | |                  |
 *          --|RST          P6.4 |<--Register Select RS
 *            |             P6.5 |<--E Clock
 *            |                  |
 *            |          Port 5  |--> Data Lines (bits 4-7)
 *            |                  |
 *            |                  |
 */
...
#define LCD_DATA        P5       //Data port -LCD Data pins (D4 - D7)
...
void lcd4bits_init(void) {
    ...
    DelayMs(20); // Increased delay for LCD startup


    lcd4bits_write(CMD_MODE, 0x33);
    DelayMs(5);
    lcd4bits_write(CMD_MODE, 0x32);  // Switch to 4-bit mode
    DelayMs(5);
...
void lcd4bits_write(unsigned char mode, unsigned char CmdChar) {
    if (mode == CMD_MODE) {//send control signals
        Set_Command_Mode;
    } else {
        Set_Data_Mode;//send character data
    }
    // Send the upper part (D7-D4)
    LCD_DATA->OUT = (LCD_DATA->OUT & 0x0F) | (CmdChar & 0xF0);//Clears the upper 4 bits while keeping the lower 4 bits unchanged.
    LCD_STROBE();

    // Send the lower part (D3-D0)
    LCD_DATA->OUT = (LCD_DATA->OUT & 0x0F) | ((CmdChar << 4) & 0xF0);//vice versa
    LCD_STROBE();
    DelayMs(2);
